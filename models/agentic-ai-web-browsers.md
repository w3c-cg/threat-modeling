...
The scope is the Computer-Using Agent (CUA) and Agentic AI Web Browsers, where it is possible to find [AI in Web API and a more general introduction](https://github.com/w3c-cg/threat-modeling/blob/main/models/ai-in-browser.md).
...
There are important implications. In particular, the agent is authorized to perform an action by the person controlling it (with all the usual identity and credential issues) to automate certain tasks.
...
For example, I can use my browser (user agent) to navigate the Catalan government website and see what public holidays are in Barcelona. I take some notes, and then I go to the W3C website, log in with my credentials, and enter the public holidays into the W3C system.


### Scripted Web Agents


However, to optimize time, I wrote a Python script that visits the Spanish government website, downloads, and parses the holiday data (luckily, there is also an .ical file, as per RFC 2445).
Then, I enter my W3C credentials into the script at runtime, the script logs into the W3C website, and enters the holidays in the correct place.

I can use Playwright, which allows me to control a browser from Python, to avoid hassles with CloudFlare and provide a simplified login experience.
...
LLMs  are far behind humans, which means they do not fully understand the concepts covered. They are good at remembering simple things, but struggle to understand more complex things (Yu et al., 2025). This is not to say that they are not useful or that they are not a fantastic business opportunity, but rather that we need to be cautious when using them, not only because they may not always provide reliable information, but also because we should not trust them with our security.

### An Agent Model

An agent, taken individually and simplified from the model used by SAIF and the OWASP AI Testing Guide, consists of several components: Application, Model, Infrastructure, and Data.
...
When conducting threat modeling, it is particularly important to note the connection points from external sources. For today, let's start by focusing on user input, which interacts with the application, as well as other agents/plugins that can provide input and output, and external sources (such as the web, which an agent can browse). 
